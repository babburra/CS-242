package Tests;

import Pieces.Board;
import Pieces.Location;
import Pieces.SpecialRook;
import org.junit.Test;

import static org.junit.Assert.assertFalse;
import static org.junit.Assert.assertTrue;

/**
 * Created by yutong on 9/19/16.
 */
public class SpecialRookTest {
    private Board board;
    @Test
    public void test() {
        board = new Board();
        SpecialRook r1 = new SpecialRook(board, new Location(0, 0), true);
        SpecialRook r2 = new SpecialRook(board, new Location(0, 7), true);
        SpecialRook r3 = new SpecialRook(board, new Location(7, 0), false);
        SpecialRook r4 = new SpecialRook(board, new Location(7, 7), false);
        board.initPiece(r1);
        board.initPiece(r2);
        board.initPiece(r3);
        board.initPiece(r4);
        assertTrue(r1.equals(board.getPiece(0, 0)));
        assertTrue(r2.equals(board.getPiece(0, 7)));
        assertTrue(r3.equals(board.getPiece(7, 0)));
        assertTrue(r4.equals(board.getPiece(7, 7)));
    }

    @Test
    public void isValid() {
        board = new Board();
        SpecialRook r1 = new SpecialRook(board, new Location(4, 4), true);
        SpecialRook r2 = new SpecialRook(board, new Location(4, 5), false);
        board.initPiece(r1);
        board.initPiece(r2);
        assertTrue("Move success", r1.validMove(4, 3));
        assertTrue("Move success", r1.validMove(7, 4));
        assertTrue("Move Success", r1.validMove(4, 6));
        assertFalse("Move fail", r1.validMove(1, 7));
    }
}
