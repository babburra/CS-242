...
public class Logic {
    ...
    public boolean noMoves(Piece p){
        ...
        board.initPiece(moveKing);
        if (isChecked(p)){
            board.movePiece(moveKing, new Location(cur_x+1, cur_y));
            if (!isChecked(p)) {
                board.removePiece(moveKing);
                return false;
            }
            board.movePiece(moveKing, new Location(cur_x-1, cur_y));
            if (!isChecked(p)){
            ...
            board.movePiece(moveKing, new Location(cur_x, cur_y+1));
            if (!isChecked(p)) {
                board.removePiece(moveKing);
                return false;
            }
            ...
            board.movePiece(moveKing, new Location(cur_x+1, cur_y+1));
            if (!isChecked(p)) {
                board.removePiece(moveKing);
                return false;
            }
    ...
    public boolean isStaleMate(Piece p){
        if ((p instanceof King) && noMoves(p) && !isCheckMate(p))
            return true;
        return false;
    }
}