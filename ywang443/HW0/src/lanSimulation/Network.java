...
public class Network {
	...
	public boolean requestBroadcast(Writer report) {
		...
		do {
			try {
				firstNode.printCurrentNode(report, "\tNode '", currentNode.name, "' accepts broadcast packet.\n");
				firstNode.printCurrentNode(report, "\tNode '", currentNode.name, "' passes packet on.\n");
				report.flush();
			} catch (IOException exc) {
				// just ignore
			}
	...
	public boolean requestWorkstationPrintsDocument(String workstation,
		...
		try {
			firstNode.printCurrentNode(report, "'", workstation, "' requests printing of '");
			firstNode.printCurrentNode(report, document, "' on '", printer);
			report.write("' ...\n");
		} catch (IOException exc) {
			// just ignore
		}
		...
		try {
			firstNode.printCurrentNode(report, "\tNode '", startNode.name, "' passes packet on.\n");
			report.flush();
		} catch (IOException exc) {
			// just ignore
		}
		...
		if (packet.destination.equals(currentNode.name)) {
			result = currentNode.printDocument(packet, report, this);
		} else {
			try {
				report
